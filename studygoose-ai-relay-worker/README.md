# StudyGoose 個人 AI Relay（Cloudflare Worker）

這個 Worker 是 GitHub Pages 版讀書鵝的 AI 安全層。它支援 GPT（OpenAI API）或 Claude（Anthropic API），並把 API Key 保留在 Cloudflare **Secret**；網頁與 GitHub 都不會保存 API Key。
...
`ALLOWED_ORIGIN` 是你的 GitHub Pages 網站的「網域」，不包含 repo 名稱。例如網站是 `https://jhen-1993.github.io/studygoose/`，仍填 `https://jhen-1993.github.io`。

## 部署方式

1. 將整個專案上傳 GitHub；本資料夾必須保留在 repo 內。
2. 在 Cloudflare **Workers & Pages** 建立 Worker，連接這個 GitHub repo。
3. 將 Root directory 設成 `studygoose-ai-relay-worker`，Build command 留白，Deploy command 填 `npm run deploy`。
...
6. 到 **Domains** 複製 Production Worker URL，例如 `https://studygoose-ai-relay.<你的帳號>.workers.dev`。
7. 回到讀書鵝網頁的「個人 AI Relay 設定」，選擇 GPT／Claude、貼上 URL 和同一組 `RELAY_ACCESS_TOKEN`，按「測試並儲存 Relay」。
...
- AI 功能包含筆記統整、出題、批改、圖片／掃描 PDF OCR，皆會使用你選擇的 API 帳號計費。
